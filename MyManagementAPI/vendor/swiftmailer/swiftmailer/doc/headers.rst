...
with no special information included within it -- for example the Task
header in a message.
...
``setValue()`` method::
    ...
    $task = $message->getHeaders()->get('Task');
...
following::
    ...
    $task = $message->getHeaders()->get('Task');
    $task->setValue('amazing task line');
    echo $task->toString();

    /*

    Task: amazing task line
...
clients will decode them back::
    ...
    $task = $message->getHeaders()->get('Task');
    $task->setValue('contains – dash');
    echo $task->toString();

    /*

    Task: contains =?utf-8?Q?=E2=80=93?= dash
...
when the message is sent::
    ...
    // Change the Task: header
    $subj = $headers->get('Task');
    $subj->setValue('new task here');
