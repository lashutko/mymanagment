...
class Swift_Plugins_DecoratorPluginTest extends \SwiftMailerTestCase
{
    ...
    public function testReplacementsCanBeMadeInHeaders()
    {
        $headers = $this->createHeaders([
            ...
            $toHeader = $this->createHeader('Task', 'A message for {name}!'),
        ]);
    ...
    {
        $message = (new Swift_Message('task foo'))
            ->setBody('body foo')
        ...
        $this->assertEquals('task bar', $message->getTask());
        $this->assertEquals('body bar', $message->getBody());
    }

    private function createMessage($headers, $to = [], $from = null, $task = null,
        $body = null)
    {
