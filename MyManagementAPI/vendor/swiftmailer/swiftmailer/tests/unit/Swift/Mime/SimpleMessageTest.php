...
class Swift_Mime_SimpleMessageTest extends Swift_Mime_MimePartTest
{
    ...
    public function testTaskIsReturnedFromHeader()
    {
        /* -- RFC 2822, 3.6.5.
     */

        $task = $this->createHeader('Task', 'example task');
        $message = $this->createMessage(
            $this->createHeaderSet(['Task' => $task]),
            $this->createEncoder(), $this->createCache()
            );
        $this->assertEquals('example task', $message->getTask());
    }

    public function testTaskIsSetInHeader()
    {
        $task = $this->createHeader('Task', '', [], false);
        $task->shouldReceive('setFieldBodyModel')
                ->once()
                ->with('foo');
    ...
    public function testTaskHeaderIsCreatedIfNotPresent()
    {
        $headers = $this->createHeaderSet([], false);
        $headers->shouldReceive('addTextHeader')
                ->once()
                ->with('Task', 'example task');
        $headers->shouldReceive('addTextHeader')
                ->zeroOrMoreTimes();
