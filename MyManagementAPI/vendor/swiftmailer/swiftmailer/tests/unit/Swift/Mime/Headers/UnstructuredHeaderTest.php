...
class Swift_Mime_Headers_UnstructuredHeaderTest extends \SwiftMailerTestCase
{
    ...
    {
        $header = $this->getHeader('Task', $this->getEncoder('Q', true));
    ...
    {
        $header = $this->getHeader('Task', $this->getEncoder('Q', true));
        $this->assertEquals('Task', $header->getFieldName());
    }
    ...
    public function testLanguageInformationAppearsInEncodedWords()
    {
        ...
        */
        ...
        $header = $this->getHeader('Task', $encoder);
        $header->setLanguage('en');
        $header->setValue($value);
        $this->assertEquals("Task: =?utf-8*en?Q?fo=8Fbar?=\r\n",
            $header->toString()
            );
    }
