      ...
      <div class="row" v-if="curators">
        <section class="col-12">
          ...
          <table class="table table-hover">
              ...
              <tr v-for="curator in curators" v-if="curator.isAdmin != true">
                  <td>{{curator.id}}</td>
                  <td>{{curator.name}}</td>
                  <td>{{curator.email}}</td>
                  <td>{{curator.tagId}}</td>
                  <td><router-link class="nav-button" :to="'/curator/edit/' + curator.id">Edit</router-link></td>
        ...
        <div class="col-12 navigation_buttons">
          <ul>
              <li><router-link class="nav-button" to="/curator/add">Add Curator</router-link></li>
          </ul>
       </div>
      </div>
    </div>
