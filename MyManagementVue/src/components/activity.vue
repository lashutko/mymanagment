          ...
          <h3>Presence Programmers: </h3>
            <table class="table table-hover">
                ...
                <tr v-for="programmer in programmers">
                    ...
                    <td>{{programmer.groupId}}</td>
                    <td><button class="delete" v-on:click=deleteProgrammer(programmer.id)>&#x274C;</button></td>
                </tr>
            </table>
            <div class="col-12 navigation_buttons">
              <ul>
                <li><router-link :to="{ name: 'addProgrammerAttandance', params: { activity_id: id, group_id: this.activity.project.group_id  } }">Add programmer</router-link></li>
              </ul>
            </div>
        </div>
        <section v-if="isChecked() && isToday()" class="row top-50">
          <h3>Check attandance list: </h3>
            <table class="table table-hover">
                ...
                <tr v-for="programmer in programmersGroup">
                    <td>{{programmer.programmer_id}}</td>
                    <td>{{programmer.name}}</td>
                    <td>{{programmer.tagId}}</td>
                    <td><button class="presence" v-on:click=addToList(programmer.programmer_id)><span v-show="isOnList(programmer.programmer_id)">&#x274C;</span></button></td>
                </tr>
            </table>
...
export default {
  ...
  methods: {
      ...
      getProgrammersAll(){
        ...
        axios.get(this.$store.getters.getUrl + 'api/groups/' + this.activity.project.group_id)
        .then(function (response) {
            this.group = response.data.data[0];
            this.programmersGroup = response.data.data[0].programmers;
            console.log(this.programmersGroup);
        }.bind(this))
        .catch((error)=>{
          this.$router.push('login'); 
        });
      ...
      addToList(id){
        ...
        this.attandanceList.push({programmer_id: id, activity_id: this.id, data_presence: dateTime});
      },
      isOnList(id){
        for(var i = 0; i < this.attandanceList.length; i++){
          if(this.attandanceList[i].programmer_id === id){
            return true;
          }
        }
        return false;
