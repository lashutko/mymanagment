       ...
       <div class="right_column col-12 col-lg-6">
        ...
        <div v-if="showSimple" class="activity simpleActivity col-12"> 
            ...
            <div class="content">
              ...
              <select v-model="activity.project_id" class="projectId">    
                <option value="0" selected>Free activity</option>   
                <option v-for="project in projects" :value="project.id">
                  {{project.name}}
                </option>                                                   
              </select>
              <select v-if="activity.project_id == 0" v-model="activity.group_id" class="projectId">    
...
export default {
  ...
  methods: {
     addActivity(isRecurrent) {
      ...
      if(this.activity.project_id === 0){
        this.activity.project_id = null;
      }
      ...
      if(isRecurrent){
        ...
        for(var i = 0; i < length; i++){
          let activityTemp = {activityDate: this.dates[i], title: this.activity.title, duration: this.activity.duration, hour:  this.activity.hour, project_id: this.activity.project_id, recurrence_id:  this.activity.recurrence_id, checked: false, group_id: this.activity.group_id, curator_id: localStorage.getItem('curator_id')};
        ...
        this.activity.curator_id = localStorage.getItem('curator_id');
    ...
    getProjectsId() {
      axios.get(this.$store.getters.getUrl + 'api/projects')
